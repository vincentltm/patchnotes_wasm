...
export class Connection {
    ...
    async sendCmd(cmd, buf) {
        ...
        try {
            ...
            try {
                const status = await this.getCommandStatus();
                if (!status.isOk()) {
                    throw new StatusError(
                        `${PicobootCmdId.toString(cmd.id())} failed: ${status.getStatusName()}`,
                        this.target,
                        status.getStatusCode()
                    );
                }
            } catch (statusErr) {
                if (statusErr instanceof StatusError) {
                    throw statusErr;
                }
                console.warn(`Unable to read command status after ${PicobootCmdId.toString(cmd.id())} failed:`, statusErr);
            }
            throw e;
        }
    }
    ...
    async checkAndClearHalts() {
        try {
            if (await this.isEndpointHalted('in', this.inEp)) {
            ...
            if (await this.isEndpointHalted('out', this.outEp)) {
